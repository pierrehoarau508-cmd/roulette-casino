...
function doSpin(onResult) {
  ...
  const seg    = 360/37;
  // Centre du segment gagnant (en degrés)
  const segCenter = idx * seg + seg / 2;
  // L'aiguille est à GAUCHE (180°). Pour y amener segCenter :
  // on veut que : (currentAngle + segCenter) % 360 == 180
  // => delta = (180 - (currentAngle % 360) - segCenter + 360*2) % 360
  const curMod = ((state.currentAngle % 360) + 360) % 360;
  let   delta  = (180 - curMod - segCenter % 360 + 720) % 360;
